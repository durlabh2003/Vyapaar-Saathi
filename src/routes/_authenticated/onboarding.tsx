...
export const Route = createFileRoute("/_authenticated/onboarding")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Set up your business — Vyapaar Saathi" },
      {
        property: "og:description",
        content: "Name your shop and start recording entries by voice.",
      },
    ],
  }),
...
function OnboardingPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      // Only attach owner_id if there is a real authenticated Supabase session
      ...
      if (createdBiz?.id) {
        await seedDemoKiranaData((createdBiz as { id: string }).id).catch(() => {});
      }
      ...
      if (ownerId) {
        try {
          ...
          // ignore profile save failure in demo mode
        }
      }
